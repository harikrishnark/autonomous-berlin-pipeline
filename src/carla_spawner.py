...
def process_image(image, client_socket):
    """
    ...
    try:
        ...
        # Encode and compress using OpenCV (imported inside to avoid global dependency issues on Mac)
        import cv2
...
def main():
    ...
    # --- 5. START STREAMING ---
    # Every time the camera takes a photo, trigger `process_image` to send it over the socket
    camera.listen(lambda image: process_image(image, ai_socket))
